...
# Example of another heuristic function
...
def aStar(start, target, cols, rows, search_space, heuristic):
    # Check if we start and target, then there is nothing to do
    if start[0] == target[0] and start[1] == target[1]:
        return ([start])
        ...
        for neighbour in get_neighbours(current, cols, rows):
            if neighbour not in parent:
                ...
                if neighbour not in queue or (current_g < g_score.get(neighbour, float('inf'))):
                    parent[neighbour] = current
                    g_score[neighbour] = current_g
                    f_score[neighbour] = current_g + heuristic(neighbour, target)
...
def main(input_file, heuristic):
    with open(input_file, 'r') as file:
        lines = file.read().split("\n")

    new_lines = [list(map(int, line.split())) for line in lines]
    in1 = new_lines[0]
    rows, cols = int(in1[0]), int(in1[1])

    search_space = new_lines[1:1 + rows]
    start_position = tuple(new_lines[rows + 1])
    end_position = tuple(new_lines[rows + 2])
    ...
    path, explored, frontier = aStar(start_position, end_position, cols, rows, search_space, heuristic)
    print(path)
    depth, cost = trace_back(path, search_space, True, heuristic)

    print(f"\nNumber of items in explored list: {explored}")
    print(f"Number of items in frontier: {frontier}")

    return depth, cost, explored, frontier
